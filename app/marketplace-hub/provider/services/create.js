// app/marketplace-hub/provider/services/create.js
// ISSY Marketplace - Create/Edit Service Screen
...
import { createService, getCategories } from '../../../../src/services/marketplaceApi';
    ...
    try {
      ...
      if (result.success) {
        Alert.alert(
          ...
          [{ text: 'OK', onPress: () => router.replace('/marketplace-hub/provider/services') }]
        );
      } else {
        Alert.alert('Error', result.error || 'No se pudo guardar el servicio');
      }
    } catch (error) {
      Alert.alert('Error', 'Ocurrió un error al guardar el servicio');
    } finally {
      setLoading(false);
    }
  };
